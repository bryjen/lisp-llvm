...
const char *file_template = "(let [name \"World\"]\n"
                            "(println (str \"Hello from \" %s \"!\")))";

const char *toml_config_template = "# Project metadata\n"
                                   "name = \"%s\"\n"
                                   "version = \"0.1.0\"\n"
...
*get_abs_dir_path(const char *const dir_path, char **out_error) {
  ...
  if (_fullpath(abs_dir_path, dir_path, _MAX_PATH) == NULL) {
    ...
    sprintf_s(buf, DEFAULT_BUFFER_SIZE, "\"%s\" is not a valid directory.",
              dir_path);
    *out_error = buf;
...
assert_directory_contains_no_files(const char *const abs_dir_path, char **out_error) {
  if (does_directory_contain_files(abs_dir_path)) {
    char *buf = malloc(DEFAULT_BUFFER_SIZE);
    sprintf_s(buf, DEFAULT_BUFFER_SIZE,
              "Cannot create a project at \"%s\", it is not empty.",
              abs_dir_path);
    *out_error = buf;
...
assert_dir_is_suitable(const char *const abs_dir_path, char **out_error) {
  if (does_directory_exists(abs_dir_path)) {
    ...
    if (!CreateDirectoryA(abs_dir_path, NULL)) {
      char *buf = malloc(DEFAULT_BUFFER_SIZE);
      sprintf_s(buf, DEFAULT_BUFFER_SIZE,
                "Cannot the directory \"%s\" with error code '%d'.",
                abs_dir_path, GetLastError());
      *out_error = buf;
      return false;
    }
  }
...
generate_sample_file(const char *const abs_dir_path, const char *const project_name, char **out_error) {
  ...
  sprintf_s(file_path, DEFAULT_BUFFER_SIZE, "%s\\%s.clj", abs_dir_path,
            project_name);
  ...
  if (err != 0 || !f) {
    ...
    sprintf_s(file_open_err, DEFAULT_BUFFER_SIZE,
              "Failed to generate the template file \"%s.clj\"", project_name);
...
generate_sample_config_file(const char *const abs_dir_path, const char *const project_name, char **out_error) {
  ...
  if (err != 0 || !f) {
    ...
    sprintf_s(file_open_err, buf_size,
              "Failed to generate the template file \"%s.toml\"", project_name);
    *out_error = file_open_err;
...
init_project(const InitArgs init_args, char **out_error) {
  ...
  char *abs_dir_path =
      get_abs_dir_path(init_args.initDirPath, &get_abs_dir_err);
  if (!abs_dir_path) {
  ...
  if (!generate_sample_file(abs_dir_path, init_args.projectName,
                            &unsuitable_dir_err)) {
  ...
  if (!generate_sample_config_file(abs_dir_path, init_args.projectName,
                                   &template_config_file_gen_err)) {
